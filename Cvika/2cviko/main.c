...
/*                                                                            */
/*  Reseni vytvoril(a) a odevzdava: (Zdenìk Brhel, xbrhel04)                  */
/*                                                                            */
/******************************************************************************/
...
void beep(int freq) {
    for (uint32_t q=0; q<300; q++) {
        ...
        GPIOB_PDOR &= ~PIEZZO_MASK; delay(300);
    }
}
...
#define BUTTON_RIGHT_MASK 0x80*/
	...
	if(PORTB_ISFR == 0x10 && !(GPIOB_PDIR & 0x10))
	{
		beep(650);
		flash();
	}
	if(PORTB_ISFR == 0x08 && !(GPIOB_PDIR & 0x08))
	{
		beep(800);
		flash();
	}
	if(PORTB_ISFR == 0x20 && !(GPIOB_PDIR & 0x20))
	{
		beep(950);
		flash();
	}
	if(PORTB_ISFR == 0x40 && !(GPIOB_PDIR & 0x40))
	{
		beep(1100);
		flash();
	}
	if(PORTB_ISFR == 0x80 && !(GPIOB_PDIR & 0x80))
	{
		beep(1200);
		flash();
	}
